class TimeEntriesController < ApplicationController
  ...
  def index
    @time_entries = TimeEntry.joins(:employee).where(employees: { user_id: current_user.id })
  end
  ...
  private
    ...
    def set_time_entry
      @time_entry = TimeEntry.joins(:employee).where(employees: { user_id: current_user.id }).find(params[:id])
    end

    # Only allow a list of trusted parameters through.
    def time_entry_params
      params.require(:time_entry).permit(:employee_id, :work_date, :hours_worked, :overtime_hours, :notes)
    end
end
